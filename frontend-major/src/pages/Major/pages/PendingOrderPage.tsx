...
function PendingOrderPage() {
  ...
  }, [theaterInfo, totalPrice, promptPayNum]);
  ...
  const handleConfirm = async () => {
    try {
      ...
      navigate(
        `/success?reserve=${reservationId}&total=${totalPrice}&seatTypes=${seatTypes}&selectedSeats=${seatWithRow}&seatIds=${seatIds}&showid=${showId}`
      );
    } catch (error) {
  ...
  const createPayment = async (reservationId: number) => {
    // Accept reservationId as an argument
    try {
