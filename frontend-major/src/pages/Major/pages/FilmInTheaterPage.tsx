...
const FilmInTheaterPage = () => {
    ...
    const [filmsOrganizedByScreen, setFilmsOrganizedByScreen] = useState<Map<string, Map<number, Film[]>>>(new Map());
    const [theaterInfo, setTheaterInfo] = useState<Theater>({theaterId: 0, name: '', address: '', phoneNum: '', promptPayNum: '', latitude: '', longitude: ''});
    const fetchShowtimes = async () => {
        try {
            ...
            const response = await Axios.get(
                `http://localhost:3000/show/getShowByTheaterIdAndDate/${id}/${dateToUse}`
            );
            console.log(id)
            console.log(dateToUse)
            console.log(selectedDate)
            ...
            response.data.forEach((theaterScreening: TheaterScreening) => {
                theaterScreening.films.forEach((film) => {
                    ...
                    if (screens) {
                        if (!screens.has(theaterScreening.screen.screen_number)) {
                            screens.set(theaterScreening.screen.screen_number, []);
                        }
                        screens.get(theaterScreening.screen.screen_number)?.push(film);
                    }
                });
            });
            setFilmsOrganizedByScreen(newGroupedFilms);
            const theaters = await Promise.all(response.data.map(async (theaterScreening: TheaterScreening) => {
                const theater = await Axios.get(`http://localhost:3000/theater/getTheaterById/${theaterScreening.screen.theaterId}`);
                return theater.data;
            }));
    ...
    return (
        <>
            ...
            {[...filmsOrganizedByScreen.entries()].map(([filmName, screensMap], index) => (
                <VStack key={index} align="stretch" p={4}>
                    <Flex direction={{ base: 'column', md: 'row' }} justify="space-around" align="center" mr="5vw">
                        <Image
                            src={screensMap.values().next().value[0].posterImg} 
                            alt="Movie Poster"
                            width={posterWidth}
                            height={posterHeight}
                            objectFit="cover"
                            mr="1vw"
                        />
                        <VStack align="stretch" spacing={3} flex="1" m={{ base: 4, md: 0 }}>
                            <Text fontSize="lg" fontWeight="bold">{filmName}</Text>
                            {[...screensMap.entries()].map(([screenNo, films], screenIndex) => {
                                const nearestFutureTime = findNearestFutureTimeForScreen(films);
                                return (
                                    <Box key={screenIndex}>
                                        <Text fontSize="md">
                                            Screen {screenNo} | {films[0].screenType} | {films[0].duration} mins
                                        </Text>
                                        <HStack spacing={4}>
                                            {films.map((film, filmIndex) => {
                                                ...
                                                let buttonStyle = isPast ? notAvailableStyle : (isNearest ? nearestStyle : futureStyle);
                                                return (
                                                    // /Screen/:filmId/:date/:showId/:theaterId
                                                    <Link to = {`/Screen/${film.filmId}/${film.date}/${film.showId}/${id}`}>
                                                    <Button key={filmIndex} size="sm" style={buttonStyle} isDisabled={isPast}>
                                                        {film.startTime}
                                                    </Button>
                                                    </Link>
                                                );
