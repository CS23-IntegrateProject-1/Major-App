  ...
  const fetchShowtimes = useCallback(async () => {
    try {
      ...
      const theaters = await Promise.all(
        response.data.map(async (theaterScreening: TheaterScreening) => {
                ...
                {[...screensMap.entries()].map(
                  ([screenNo, films], screenIndex) => {
                    const nearestFutureTime =
                      findNearestFutureTimeForScreen(films);
                    return (
                      <Box key={screenIndex}>
                        <Text fontSize="md">
                          Screen {screenNo} | {films[0].screenType} |{" "}
